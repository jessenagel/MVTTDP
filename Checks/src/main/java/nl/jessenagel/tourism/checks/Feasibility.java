        ...
        for (User user : area.users) {
            ...
            for (int i = 0; i < user.schedule.size(); i++) {
                ...
                if(visitedEvents.contains(batch.event)){
                    System.out.println("INFEASIBLE: Event visited twice: " + batch.event.name + " by user " + user.name);
                    return false;
                }
                ...
                if (i == 0) {
                    if (TouristTime.greater(user.startTime.increaseBy(area.travelTimes.get(user.start).get(batch.event.entrance)), batch.startTime)) {
                        System.out.println("INFEASIBLE: The first event cannot be reached in time from the starting location.");
                        return false;
                    }
                    continue;
                }
                ...
                if (TouristTime.greater(previousBatch.endTime.increaseBy(area.travelTimes.get(previousBatch.event.exit).get(batch.event.entrance)), batch.startTime)) {
                    System.out.println("INFEASIBLE: The nth event cannot be reached in time from the n-1th event. n = " + i);
                    System.out.println("Current event: " + batch.event.name);
                    batch.startTime.print();
                    System.out.println("Previous event:"+ previousBatch.event.name);
                ...
                if (i == user.schedule.size() - 1) {
                    if (TouristTime.greater(batch.endTime.increaseBy(area.travelTimes.get(batch.event.exit).get(user.end)), user.endTime)) {
                        System.out.println("INFEASIBLE: The final location cannot be reached in time from the last event.");
                        return false;
                    }
                }
            }
        ...
        for (Event event : area.events.values()) {
            for (int t = 0; t <= 1440; t++) {
                ...
                if(sum > event.capacity){
                    System.out.println("INFEASIBLE: the concurrent capacity at event " + event.name + " is exceed by scheduling " + sum + " users while the capacity is " + event.capacity + " at time:");
        ...
        for(Event event : area.events.values()){
            for(Batch batch : event.batches){
                if(batch.getCurrentVisitors() > batch.capacity){
                    System.out.println("INFEASIBLE: the capacity of a batch at event " + event.name + " is exceed. This batch has " + batch.getCurrentVisitors() + " visitors and only " + batch.capacity + " capacity. The batch starts at:");
                    batch.startTime.print();
                    return false;
                }
            }
